...
describe("toApiMessages", () => {
  it("debería convertir role 'character' a 'model'", () => {
    // Arrange
    const messages = [{ role: "character", text: "Hola, soy Mickey" }];

    // Act
    const result = toApiMessages(messages);

    // Assert
    expect(result).toEqual([{ role: "model", parts: [{ text: "Hola, soy Mickey" }] }]);
  });

  it("debería mantener role 'user' como 'user'", () => {
    const messages = [{ role: "user", text: "Hola Mickey" }];
    const result = toApiMessages(messages);
...
describe("normalizeAIResponse", () => {
  it("debería extraer el texto de una respuesta válida de Gemini", () => {
    const rawResponse = {
      candidates: [
        { content: { parts: [{ text: "¡Uh-oh! Hola!" }] } },
      ],
    };

    const result = normalizeAIResponse(rawResponse);

    expect(result).toBe("¡Uh-oh! Hola!");
  });

  it("debería unir varias parts en un solo string", () => {
    const rawResponse = {
      candidates: [
        { content: { parts: [{ text: "Hola " }, { text: "Mickey" }] } },
      ],
    };

    expect(normalizeAIResponse(rawResponse)).toBe("Hola Mickey");
  });
...
describe("buildPayload", () => {
  it("debería armar el payload con el shape correcto para Gemini", () => {
    const payload = buildPayload({
      systemPrompt: "Sos Mickey Mouse",
      uiMessages: [{ role: "user", text: "Hola" }],
    });

    expect(payload.systemInstruction).toEqual({ parts: [{ text: "Sos Mickey Mouse" }] });
